...
/**
 * Resolves display names and parent hierarchies for JUnit test contexts. Handles @DisplayName
 * annotations, nested test classes, and parameterized tests.
 */
public class DisplayNameResolver {
  ...
  /**
   * Resolves the display name for a specific test run. For parameterized tests, preserves the
   * dynamic run name (e.g., "Run 1 with value=A"). Falls back to @DisplayName annotation or method
   * name.
   *
   * @param context the extension context
   * @param methodName the method name (must not be null)
   * @param rawDisplayName the raw display name from JUnit (may be null)
   * @return resolved display name, never null
   */
  ...
  /**
   * Resolves the static method-level display name from @DisplayName annotation. Returns the method
   * name if no annotation is present.
   */
  ...
  /**
   * Resolves the class-level display name from @DisplayName annotation. Returns the simple class
   * name if no annotation is present.
   */
  ...
  /**
   * Builds the parent chain from the root test class down to (but not including) the current test
   * class. This handles nested test classes properly.
   *
   * <p>For example, if the hierarchy is: - NestedTestsExample (top-level) - FirstNested -
   * SecondLevelNested (current)
   *
   * <p>This returns: ["NestedTestsExample", "FirstNested"]
   *
   * <p>The current class should be added separately by the caller.
   */
  ...
  /**
   * Checks if a display name is auto-generated by JUnit (not custom). JUnit generates names like
   * "methodName()" or "methodName(Type)" for methods.
   */
