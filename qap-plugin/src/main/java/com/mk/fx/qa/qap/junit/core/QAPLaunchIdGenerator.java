...
/**
 * Generates and manages unique launch identifiers for test runs. Launch IDs are stored as system
 * properties and follow the format: "base-uuid" where base is a configurable prefix and uuid is a
 * 12-character random string.
 *
 * <p>Thread-safety: generateIfAbsent() is synchronized to prevent races in parallel test execution.
 */
public class QAPLaunchIdGenerator {
  ...
  /**
   * Pre-compiled regex pattern for launch ID validation. Matches format:
   * "anystring-12+alphanumericcharacters" Compiled once for performance in high-concurrency
   * scenarios.
   */
  ...
  /**
   * Checks if the given value is a complete launch ID (contains base + UUID suffix). Uses
