...
/**
 * Simple in-memory implementation of JUnit's ExtensionContext.Store for unit testing without
 * relying on real JUnit stores.
 */
public class InMemoryStore implements ExtensionContext.Store {
  ...
  public <K, V> V getOrComputeIfAbsent(
      K key, Class<V> requiredType, Function<K, V> defaultCreator) {
    Object existing = map.get(key);
    if (existing != null) return (V) existing;
    V created = defaultCreator.apply(key);
    map.put(key, created);
