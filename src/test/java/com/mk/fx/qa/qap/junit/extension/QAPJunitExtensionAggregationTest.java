...
/**
 * Verifies that the extension aggregates nested test results under the top-level class and only
 * serializes once at the top-level afterAll.
 */
class QAPJunitExtensionAggregationTest {
  ...
  void aggregatesNestedAndEmitsOnceAtTopLevel() throws Exception {
    ...
    when(root.getStore(any()))
        .thenAnswer(inv -> stores.computeIfAbsent(inv.getArgument(0), ns -> new MapBackedStore()));
    ...
    QAPJunitLaunch launch =
        StoreManager.getClassStoreData(top, QAPUtils.TEST_CLASS_DATA_KEY, QAPJunitLaunch.class);
